...
def check_condition(l, assert_stmt_str):
	...
	for rf_edge in rf1:
		for i in range(1,11):
			if rf_edge[1][2:4]=='r{}'.format(i):
				r[i] = int(rf_edge[0].split('=')[-1])

	assert_conditions_true = are_assert_conditions_true(assert_stmt_str,r)
...
def are_assert_conditions_true(assert_stmt_str, r):
	f = open('temp.py', 'w')
	for i in range(1, 11):
		f.write('r{}={}\n'.format(i, r[i]))
